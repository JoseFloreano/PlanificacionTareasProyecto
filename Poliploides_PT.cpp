...
/*
 ScenarioData
 Representa los datos completos del escenario de planificación.

 numOperations: Número total de operaciones disponibles en el escenario.
 numMachines: Número total de máquinas disponibles.
 numJobs: Número total de trabajos definidos.

 processingTime: Matriz de tiempos de procesamiento.
    processingTime[i][j] indica el tiempo requerido para procesar
    la operación i en la máquina j.

 energyCost: Matriz de costos energéticos.
    energyCost[i][j] indica el costo de energía asociado a procesar
    la operación i en la máquina j.

 jobs: Lista de trabajos definidos en el escenario.
    Cada trabajo contiene una secuencia ordenada de operaciones.

 chromosomeMapping: Mapa que relaciona genes del cromosoma con pares (Job, Operation).

 ScenarioData(): Constructor por defecto que inicializa valores básicos del escenario.
*/
...
/*
 ConfigData
 Representa los datos de configuración del algoritmo genético.

 escenarioFile: Ruta al archivo de escenario de planificación.
 numChromosomes: Número de cromosomas por individuo.
 numGenerations: Número de generaciones de evolución.
 populationSize: Número de individuos en la población.
 mutationRate: Tasa de mutación aplicada durante la evolución.
 crossoverRate: Tasa de cruce aplicada durante la evolución.
 elitismRate: Tasa de elitismo aplicada durante la evolución.
 ConfigData(): Constructor por defecto que inicializa valores básicos de configuración.
*/
struct ConfigData
{
    string escenarioFile;
    int numChromosomes;
    int numGenerations;
    int populationSize;
    float mutationRate;
    ...
    float elitismRate;
    ConfigData()
        : escenarioFile("Escenarios/Escenario1.txt"), numChromosomes(6),
          numGenerations(100), populationSize(50),
          mutationRate(0.01f), crossoverRate(0.8f),
          elitismRate(0.1f) {}
}
    ...
    for (string line; getline(file, line);)
    {
        ...
        if (getline(ss, key, '='))
        {
            ...
            if (getline(ss, value))
            {
                ...
                if (key == "escenarioFile")
                    data.escenarioFile = value;
                else if (key == "numChromosomes")
                    data.numChromosomes = stoi(value);
                else if (key == "numGenerations")
                    data.numGenerations = stoi(value);
                else if (key == "populationSize")
                    data.populationSize = stoi(value);
                else if (key == "mutationRate")
                    data.mutationRate = stof(value);
                else if (key == "crossoverRate")
                    data.crossoverRate = stof(value);
                else if (key == "elitismRate")
                    data.elitismRate = stof(value);
            }
        }
    }
    file.close();
...
int main()
{
    try
    {
        ...
        ConfigData config = loadConfigData("config.txt");
        string filename = config.escenarioFile;
        ...
        cout << "Rango de genes: [1, " << scenario.numMachines << "]" << endl
             << endl;
